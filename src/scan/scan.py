...
from utils.evaluate_utils import get_predictions, scan_evaluate, hungarian_evaluate, ScanPredictProba
...
def scan_model(base_dataset, n_clusters=5, epoch=10, num_head=3,
                batch_size=16, num_workers=2, learning_rate=1e-4,
                lr_decay_rate=10, entropy_weight=1):
    p = {  
        ...
        "scheduler": "cosine",
        "scheduler_kwargs": {'lr_decay_rate': lr_decay_rate},
        "scan_model": "./scan_model.pt",
        "scan_checkpoint": "./scan_checkpoint.pt",
        "scan_dir": None,
        "criterion": "scan",
    ...
    # train_dataset = get_train_dataset(p, None, 
    #                                     split='train', to_neighbors_dataset = True)
    # val_dataset = get_val_dataset(p, None, to_neighbors_dataset = True)
    train_embs, test_embs = train_test_split(base_dataset, test_size=0.2)
    train_dataset = NeighborsDataset(train_embs)
    val_dataset = NeighborsDataset(test_embs)
    # size = len(nn_dataset)
    # train_dataset, val_dataset = torch.utils.data.random_split(nn_dataset, (int(0.8*size), size - int(0.8*size)))
    ...
    # backbone = None
    model = ClusteringModel(base_dataset.shape[1], p['num_classes'], p['num_heads'])
    ...
    optimizer = get_optimizer(model, p, cluster_head_only=p["update_cluster_head_only"])
    # print(optimizer)
    ...
    for epoch in range(start_epoch, p['epochs']):
        ...
        predictions = get_predictions(p, val_dataloader, model, return_features=False)
