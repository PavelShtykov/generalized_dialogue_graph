...
def get_predictions(p, dataloader, model, return_features=False):
    ...
    for batch in dataloader:
        ...
        # bs = images.shape[0]
        output = model(images, forward_pass='head')
        # if return_features:
...
class ScanPredictProba():
    ...
    def predict_proba(self, X, y=None):
        embds = X
        with torch.no_grad():
            embds = torch.from_numpy(embds).cuda()
            embds.cuda()
            output = self.model(embds)
            output = [F.softmax(h, dim=1).detach() for h in output]
